...
public sealed class MedicinesController(MedicinesService service) : ControllerBase
{
    ...
    [Authorize(Roles = "ADMIN,DOCTOR"), HttpPatch("{id:guid}/archive")] public async Task<IActionResult> Archive(Guid id, CancellationToken ct) { if (!UserId(out var userId)) return Unauthorized(); return await service.SetActiveAsync(id, false, userId, ct) ? NoContent() : NotFound(); }
    [Authorize(Roles = "ADMIN,DOCTOR"), HttpPatch("{id:guid}/restore")] public async Task<IActionResult> Restore(Guid id, CancellationToken ct) { if (!UserId(out var userId)) return Unauthorized(); return await service.SetActiveAsync(id, true, userId, ct) ? NoContent() : NotFound(); }
