...
{
    public async Task<object> SearchAsync(string? search, bool includeInactive, bool inStockOnly, int page, int pageSize, CancellationToken ct)
    {
        ...
        if (inStockOnly)
        {
            ...
            q = q.Where(x => x.Batches.Any(b => b.IsActive && b.QuantityRemaining > 0 && (b.ExpiryDate == null || b.ExpiryDate > today)));
        }
    ...
    public async Task<object> CreateAsync(SaveMedicineRequest request, Guid userId, CancellationToken ct)
    {
        ...
        if (request.InitialBatch is not null) ValidateBatch(request.InitialBatch.Quantity, request.InitialBatch.UnitCost, request.InitialBatch.SellingPrice, request.InitialBatch.ExpiryDate);
        ...
        if (request.InitialBatch is not null) { var first = request.InitialBatch; var batch = new MedicineBatch { MedicineId = medicine.Id, BatchNumber = ResolveBatchNumber(first.BatchNumber), Supplier = first.Supplier?.Trim(), QuantityReceived = first.Quantity, QuantityRemaining = first.Quantity, UnitCost = first.UnitCost, SellingPrice = first.SellingPrice, ExpiryDate = first.ExpiryDate, ReceivedById = userId }; db.MedicineBatches.Add(batch); db.StockTransactions.Add(new StockTransaction { MedicineId = medicine.Id, MedicineBatchId = batch.Id, Type = MovementType.PURCHASE, QuantityChange = first.Quantity, BalanceAfter = first.Quantity, Reason = "Opening stock recorded with medicine", PerformedById = userId }); }
    ...
    public async Task<object> AddBatchAsync(Guid medicineId, AddBatchRequest request, Guid userId, CancellationToken ct)
    {
        ...
        var batchNumber = ResolveBatchNumber(request.BatchNumber);
        if (await db.MedicineBatches.AnyAsync(x => x.MedicineId == medicineId && x.BatchNumber == batchNumber, ct)) throw new ArgumentException("This stock reference already exists for the selected medicine.");
        await using var tx = await db.Database.BeginTransactionAsync(ct);
        var batch = new MedicineBatch { MedicineId = medicineId, BatchNumber = batchNumber, Supplier = request.Supplier?.Trim(), QuantityReceived = request.Quantity, QuantityRemaining = request.Quantity, UnitCost = request.UnitCost, SellingPrice = request.SellingPrice, ExpiryDate = request.ExpiryDate, ReceivedById = userId };
    ...
    private static void ValidateBatch(decimal quantity, decimal unitCost, decimal sellingPrice, DateOnly? expiryDate)
    {
    ...
    private static string ResolveBatchNumber(string? supplied) => string.IsNullOrWhiteSpace(supplied)
        ? $"LOT-{DateTime.UtcNow:yyyyMMdd}-{Guid.NewGuid().ToString("N")[..6].ToUpperInvariant()}"
        : supplied.Trim();
